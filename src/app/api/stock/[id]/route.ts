...
export async function GET(
  ...
  try {
    ...
    const item = await db.selectFrom('stock').selectAll()
      .where('id_stock', '=', Number(id)).executeTakeFirst();
...
export async function PUT(
  ...
  try {
    ...
    await db.updateTable('stock').set(body)
      .where('id_stock', '=', Number(id)).execute();
...
export async function DELETE(
  ...
  try {
    ...
    const db = getDbForCountry(session.user.pais!);
    await db.deleteFrom('stock').where('id_stock', '=', Number(id)).execute();
    return NextResponse.json({ success: true });
  } catch (e) { return handleApiError(e); }
}