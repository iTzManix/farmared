...
export async function POST(request: NextRequest) {
  try {
    const session = await getSession();
    if (!session?.user) return NextResponse.json({ error: 'No autorizado' }, { status: 401 });
    if (!isSuperAdmin(session)) return NextResponse.json({ error: 'Solo superadmin' }, { status: 403 });

    const body = await request.json();
    const db = getAllDbs().find(d => d.pais === body.pais)?.db;
    if (!db) return NextResponse.json({ error: 'Pas invlido' }, { status: 400 });
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    await db.deleteFrom('sucursal')
      .where('id_sucursal', '=', id).executeTakeFirst();
    return NextResponse.json({ success: true });
  } catch (e) { return handleApiError(e); }
}