...
export default function ClientesPage() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/clientes?page=${page}&pageSize=20`).then(r => r.json()).then(r => { setData(r.data ?? []); setTotal(r.total ?? 0); }).finally(() => setLoading(false));
  }, [page]);
  ...
  return (
    <div className="space-y-6">
      ...
      : data.length === 0 ? <EmptyState title="Sin clientes" />
